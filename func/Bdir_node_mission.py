...
class Routing_Node_CPU_New(Routing_Node_CPU):
    def __init__(self, id, num_class=5, generate_rate=0.1, recv_rate=0.1, send_rate=0.1, processing_rate=0.1,
        ...
        self.data_monitor_recv = DATA_Time_Capacity_Monitor(tm_capacity_s=tm_capacity_s * processing_rate,
                                                            alarm_cap=alarm_cap, aer_accelerate_cap=aer_accelerate_cap)
        if is_Bdir or is_AER:
            self.bdir = BDIR()
        else:
            self.bdir = None
    ...
    def bdir_ask_alarm_state(self, data=None):  # 此处需要允许同层连接
        ''' 不能高频率的询问 '''
        if self.cloud_name == self.node_id: return False  # 云节点没必要询问，因为没有更上层的节点
        if time.time() - self.ask_bdir < self.processing_rate * 10: return False
        if self.data_monitor_recv.is_alarm_cap() > 0 and \
                (self.recv_buffer.qsize() + self.unpack_recv_buffer.qsize()) \
                < self.recv_buf_size * self.alarm_buf_rate: return False
    ...
    def bdir_process_feedback(self, feedback):
        ...
        if state > 0:
            ...
            self.bdir.remove_free_node(ori_name, self_node=self.node_id)
        return True
        pass

    def bdir_change_buffer(self):
        if self.unpack_recv_buffer.qsize() > 0:
            return self.unpack_recv_buffer
        else:
    ...
    def bdir_package_recv_iot_data(self):
        ...
        if not self.bdir: return False
        if self.cloud_name == self.node_id: return False
        free_nodes = []
        if self.is_AER and self.data_monitor_recv.is_need_assi_accelerate() < 0:
        ...
        # 高并发触发打包，容量紧张出发打包
        if self.data_monitor_recv.is_alarm_cap() < 0 or \
                (self.recv_buffer.qsize() + self.unpack_recv_buffer.qsize()) > self.recv_buf_size * self.alarm_buf_rate:
            free_nodes += self.bdir.get_max_free_node(direction='up', num=1)

        # if self.node_id[0] == '2' and self.recv_buffer.qsize()==30:
        #     for i in range(30):
        #         dat = self.recv_buffer.get()
        #         print(self.node_id, f'recv buff [{i}]:', dat)
        ...
        for idx, free_node_cost in enumerate(free_nodes):
            ...
            pack = {}
            if idx < BDIR_num:
                package_size = self.small_package_size
            else:
                package_size = self.package_size
            if package_size < 0:
                num = self.recv_buf_size * 0.5 * cap  ## 动态调节速率
            else:
                num = package_size  # 固定速率
            i = 0
            while i < num:
                recv_buffer = self.bdir_change_buffer()
                if recv_buffer.qsize() <= 0:
                    isout = True
                    break
                ...
                if not self.isMDSR:
                    try:
                        ...
                        if data_bdir['type'] == 'bdir-pack':  # 将未解压的包合并
                            packed = data_bdir['param']
                            for cate in packed:
                                if cate not in pack.keys():
                                    pack[cate] = packed[cate]  ## 每类数据独立打包
                                else:
                                    pack[cate] += packed[cate]
                            pack_cap += len(str(packed))
                            pack_num += 1
                            time.sleep(self.processing_rate * 0.001)  # 打包消耗
                            i += 1
                            continue
                    except:
                ...
                is_packed = False
                if da['type'] == 'back':
                    # print(self.node_id, '[]', da)
                    if da['target']==self.cloud_name: is_packed=True
                if da['type'] == 'data' or is_packed:  ## 只有未处理数据才打包
                    if cate not in pack.keys():
                        pack[cate] = [data]  ## 每类数据独立打包
                    else:
                        pack[cate].append(data)
                    ...
                    # self.unpack_recv_buffer.put(data)   #  不能立即放回去，否者会一直死循环卡在这条数据上
                if time.time() - start_tm > self.max_pack_time_s:
                    # print('bdir_package_recv_iot_data:', self.node_id, 'package timeout!',
                    #       f'target:{free_node} packed:{pack_num}', flush=True)
                    # print('bdir_package_recv_iot_data package timeout!:', free_nodes, 'recv buffer size:', self.recv_buffer.qsize(), data )
                    break  # 打包时间限制
            ...
            if len(list(pack.keys())) > 0:
                ...
                assi_num += [free_node]
                print('\nbdir_package_recv_iot_data:', f'{self.node_id}-->{free_node}',
                      f'capacity:{pack_cap}  pack:{i}/{num}   category:',
            ...
            if time.time() - start_tm0 > (self.assi_nodes_num) * self.max_pack_time_s or isout: break

        if len(assi_num) > 0:
            writedata = {'direction': 'send', 'start time': start_tm0, 'end time': time.time(),
        ...
        for d in temp_buff:
            self.unpack_recv_buffer.put(d)  # 读过的数据，重新放回去， self.recv_buffer内存小，暂不优先
        return True
        pass
    ...
    def bdir_unpack_recv_package(self, data):
        ''' 数据包拆解，应在接收线程内调用 '''
        pack = queue.Queue()
        if self.unpack_recv_buffer.qsize() > (
                self.recv_buf_size * 2 * max(self.package_size, self.small_package_size)): return False
        if not self.bdir: return False
        ...
        try:
            if data['type'] != 'bdir-pack': return False
        except:
        ...
        logprint.node_record_alldata(self.bdir_log, writedata)
        print('bdir_unpack_recv_package:', f'{self.node_id} <-- {ori_node}', f'capacity:{pack_cap}  num={pack_num}',
              flush=True)
    ...
    def node_recv_data_(self):
        ...
        if recv_data is not None:  ## 模拟有数据发送到缓存区
            ...
            if self.massage_processing(recv_data): return True
            ...
            if len_size + size > self.recv_buf_size * self.alarm_buf_rate:
                rate = ((len_size + size) - self.recv_buf_size * self.alarm_buf_rate) / self.recv_buf_size * 100  ## 反馈调整成本大小可调
    ...
    def node_processing_data_thread(self, dat: {} = None, real_thread: bool = True):
        ...
        while True:
            ...
            if (self.recv_buffer.qsize()) > 0:
                dat = self.recv_buffer.get(timeout=1e-3)  ## 优先处理实时接收数据
                da = self.bdir_unpack_recv_package(dat)  # 数据压缩包解压,非压缩包就重新存入self.unpack_recv_buffer
                if da:  # dat 是数据包
                    if self.unpack_recv_buffer.qsize() > 0:
                        dat = self.unpack_recv_buffer.get(timeout=1e-3)
                else:
                    pass
            else:  # 接收缓存是空，开始处理解压包
                if self.unpack_recv_buffer.qsize() > 0:
            ...
            if dat is not None:
                # if not self.massage_processing(dat): ### 带宽回复，在接收线程进行

                if self.isIoT:
                    if not self.iot_data_processing(dat):  ## 不是目标数据，重新处理
                        try: self.recv_buffer.put(dat, timeout=1e-3)
                        except: self.unpack_recv_buffer.put(data, timeout=1e-3)
                else:
            ...
            if self.node_id != self.cloud_name and not self.isIoT and self.isbalance \
                ...
                if da:  # data是数据包
                    if self.unpack_recv_buffer.qsize() > 0:
                        buffer = self.unpack_recv_buffer
                    else:
                        buffer = self.recv_buffer
                    data = buffer.get(timeout=1e-6)

                # if not self.massage_processing(data):  ### 带宽回复，在接收线程进行
                ...
                if da['type'] == 'data':  ## 只有iot数据才能立即转发
                    ...
                    if self.node_send_data_(name_id=name_id, dat=data):  ## 给上层处理
                        ...
                        try: self.recv_buffer.put(data, timeout=1e-3)
                        except: self.unpack_recv_buffer.put(data, timeout=1e-3)
                else:
                    try: self.recv_buffer.put(data, timeout=1e-3)
                    except: self.unpack_recv_buffer.put(data, timeout=1e-3)
            ...
            if self.node_id == self.cloud_name:  ##云节点的无敌运算能力
                ...
                while self.generate_buffer.qsize() > 0:
                    ...
                    #     sleep(1e-12)
                    pass
            if time.time() - tm > self.processing_rate*100:
                tm = time.time()
                self.node_processing_func_1_res_buff_vishow()
