...
class Routing_Node_CPU(object):
    ...
    def node_send_data_thread(self, dat=None, real_thread: bool = True):
        ...
        while True:  # self.all_thread_run():
            ...
            if self.send_buffer.qsize() == 0:
                ...
                #     print()
                # (dat, target) = data
                # if self.cloud_name == self.node_id: print(self.node_id, '-->', target, ':', dat, flush=True)
                ...
                if target is None:
                    with self.mux_lock_connect_cost:
                        cost_dict = self.node_connect_cost_dict.copy()
                ...
                # if abs(int(self.node_id[0])- int(target[0]))!=1 : print(self.node_id, '-2->:', target, 'targ:', target2, dat)
                if target is None:
    ...
    def node_processing_data_thread(self, dat: {} = None, real_thread: bool = True):
        ...
        while True:
            ...
            if (
                    len_size > 0 and self.isIoT) or len_size > self.generate_buf_size * self.alarm_buf_rate:  ###      iot数据一律上传    不仅是为了展示效果
    ...
    def node_report_back(self, ori_dat: {} = None, rate: float = 0):
        ...
        with self.mux_lock_connect_cost:
            self.node_connect_cost_dict[node_id][idx_class] = int(
                self.node_connect_cost_dict[node_id][idx_class]) + int(
                rate)  # 调整k类数据通信成本
            self.node_connect_cost_dict[node_id][idx_class] = max(200, self.node_connect_cost_dict[node_id][
                idx_class])
        ...
        if 'cost' == dat['type']:  # 调整k类数据通信成本
            ...
            with self.mux_lock_connect_cost:
                self.node_connect_cost_dict[ori_id][idx_class] = int(
                    self.node_connect_cost_dict[ori_id][idx_class]) + int(
                    value)
                self.node_connect_cost_dict[ori_id][idx_class] = max(200, self.node_connect_cost_dict[ori_id][
                    idx_class])
            return True  # 处理完了本条指令
                    ...
                    if int(node[:-7]) - 1 == int(self.node_id[:-7]):  # 默认每层节点数量<99  # 只向下层节点传递，避免重复发送降本指令，造成网络震荡
                        with self.mux_lock_connect_cost:
                            self.node_connect_cost_dict[node][idx_class] = int(
                                self.node_connect_cost_dict[node][idx_class]) + int(
                                value)
                            self.node_connect_cost_dict[node][idx_class] = max(200,
                                                                                  self.node_connect_cost_dict[node][
                                                                                      idx_class])
                ...
                if 'data' == dat['type']:  # 调整k类数据通信成本
                    ...
                    with self.mux_lock_connect_cost:
                        self.node_connect_cost_dict[node_id][idx_class] = int(
                            self.node_connect_cost_dict[node_id][idx_class]) + int(
                            -500)
                        self.node_connect_cost_dict[node_id][idx_class] = max(200, self.node_connect_cost_dict[node_id][idx_class])
    ...
    def get_all_data_processed(self):
        ...
        num = np.array(num)
        # def fun(recv_buffer, name):
        #     if recv_buffer.qsize()>0:
        #         self.show = True
        #         print(self.node_id, f' {name}, [{recv_buffer.qsize()}]')
                # for i in range(recv_buffer.qsize()):
                #     dat = recv_buffer.get()
                #     print(self.node_id, f' {name}, [{recv_buffer.qsize()}]', dat)
                #     recv_buffer.put(dat)
        # fun(self.recv_buffer, 'self.recv_buffer')
        # fun(self.send_buffer, 'self.send_buffer')
        return num
